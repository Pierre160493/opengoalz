...
DO $$
DECLARE
    v_club_id INT := 3577; -- Variable for the selected club
...
BEGIN
    ...
    FOR rec_scenario IN
        SELECT * FROM (VALUES
            ...
            (1, 5000, -- Club's initial cash
            5000, -- Revenues from sponsors
            2000, -- Training expenses target
            1000, -- Scouts expenses target
            100, -- Expected player expenses
            100, -- Missed expenses
            0) -- Missed expenses to pay in priority
            -- Scenario 2: Club cant pay all expenses
            ,(2, 0, -- Club's initial cash
            99, -- Revenues from sponsors
            500, -- Training expenses target
            1000, -- Scouts expenses target
            200, -- Expected player expenses
            10, -- Missed expenses
            5)  -- Missed expenses to pay in priority
            -- Scenario 3: Club cant pay all expenses
            ,(3, 0, -- Club's initial cash
            150, -- Revenues from sponsors
            500, -- Training expenses target
            1000, -- Scouts expenses target
            100, -- Expected player expenses
            100, -- Missed expenses
            5)  -- Missed expenses to pay in priority
        ) AS t(number, club_cash, revenues_sponsors, expenses_training_target, expenses_scouts_target, expenses_expected, expenses_missed, expenses_missed_to_pay_in_priority)
    LOOP
    RAISE NOTICE '###### Running scenario: %', rec_scenario.number;

        -- Directly update the club's fields with the scenario values
        UPDATE clubs SET
            cash = rec_scenario.club_cash,
            revenues_sponsors = rec_scenario.revenues_sponsors,
            revenues_transfers_done = 0,
            revenues_transfers_expected = 0,
            expenses_training_target = rec_scenario.expenses_training_target,
            expenses_scouts_target = rec_scenario.expenses_scouts_target,
            expenses_transfers_done = 0,
            expenses_transfers_expected = 0
        WHERE id = v_club_id;

        -- Directly update the player's fields with the scenario values
        UPDATE players SET
            expenses_expected = rec_scenario.expenses_expected,
            expenses_missed = rec_scenario.expenses_missed,
            expenses_missed_to_pay_in_priority = rec_scenario.expenses_missed_to_pay_in_priority,
            expenses_payed = 0
        ...
        v_club_cash := rec_scenario.club_cash + rec_scenario.revenues_sponsors;

        -- Player's expenses payed
        v_expenses_payed := LEAST(
            v_club_cash, 
            rec_scenario.expenses_expected + rec_scenario.expenses_missed);
        -- Player's expenses missed left after weekly payment
        v_expenses_missed := rec_scenario.expenses_missed + rec_scenario.expenses_expected - v_expenses_payed;
        ...
        END IF;
        IF rec_tmp.expenses_players != v_expenses_payed THEN
            RAISE EXCEPTION 'Club expenses_players incorrect: expected %, got %', v_expenses_payed, rec_tmp.expenses_players;
        END IF;
