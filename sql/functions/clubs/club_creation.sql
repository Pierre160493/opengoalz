-- DROP FUNCTION public.club_creation(int8, int8, continents, int8);

CREATE OR REPLACE FUNCTION public.club_creation(inp_id_multiverse bigint, inp_id_league bigint, inp_continent continents, inp_number bigint)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- INSERT new bot club
    INSERT INTO clubs (id_multiverse, id_league, id_country, pos_league)
        VALUES (inp_id_multiverse, inp_id_league, loc_id_country, inp_number)
    ...
    loc_id_player := players_create_player(
        inp_id_multiverse := inp_id_multiverse,
