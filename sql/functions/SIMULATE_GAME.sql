...
CREATE OR REPLACE FUNCTION public.simulate_game(inp_id_game integer DEFAULT NULL::integer)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    ------------ Checks
    ------ Get the count of rows where id = inp_id_game
    ...
    IF rec_game.is_played THEN
        RAISE EXCEPTION 'Game with id % has already been played', inp_id_league;
    END IF;
    ...
    FOR loc_period_game IN 1..4 LOOP
        ...
        -- You may adjust the probabilities based on your specific requirements
            ...
            IF random() < loc_goal_probability THEN
                ...
                END IF;
                SELECT id INTO loc_id_event_type FROM game_events_type WHERE type = 'goal' ORDER BY RANDOM() LIMIT 1; -- Select the id of a random goal event
                ...
                VALUES (
                    ...
                    loc_date_start_period + (loc_minute_game * INTERVAL '1 minute') -- The date and time of the event
                );
