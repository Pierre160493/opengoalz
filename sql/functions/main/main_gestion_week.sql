...
BEGIN
    ...
    FOR multiverse IN (SELECT * FROM multiverses ORDER BY id) LOOP
        RAISE NOTICE '****** MAIN: Multiverse [%] S%W%D%: date_now= %', multiverse.name, multiverse.season_number, multiverse.week_number, multiverse.number_day, multiverse.date_now;
        ...
        WHILE now() > multiverse.date_now LOOP
            ...
            ------ Try to simulate the games of the current week
...
--RAISE NOTICE '=> Took: % seconds', EXTRACT(EPOCH FROM (clock_timestamp() - start_time));
        ...
        END LOOP; -- End of the WHILE LOOP now is greater than current multiverse time
