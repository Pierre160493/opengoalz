...
class PlayerService {
  Stream<List<Player>> getPlayersStream(List<int> playerIds,
    ...
    var stream = supabase
              ...
              .map((List<TransferBid> transfersBids) {
                for (Player player in players) {
                  ...
                  player.transferBids.addAll(
                      transfersBids.where((bid) => bid.idPlayer == player.id));
                }
                return players;
              });
        })
        .switchMap((List<Player> players) {
          ...
          return supabase
              ...
              .inFilter('id', clubIds)
              .map(
                  (maps) => maps.map((map) => Club.fromMap(map, user)).toList())
              .map((List<Club> clubs) {
                for (Player player in players.where((p) => p.idClub != null)) {
                  player.club =
  ...
  Stream<List<Player>> getPlayersStreamWithCriterias(
      PlayerSearchCriterias searchCriterias, Profile user) {
    return Stream.fromFuture(searchCriterias.fetchPlayerIds())
        .switchMap((playerIds) {
      return getPlayersStream(playerIds, searchCriterias, user);
    });
  }
}
